...
namespace SDL_Wrap {
	...
	Window::Window(int const& width, int const& height, char* const & title) {
		...
		m_surface = Surface(SDL_GetWindowSurface(m_window.get()));
	}


	Window::~Window() {
		// SDL_DestroyWindow(m_window.g);
		// m_surface.~Surface();
	}
	...
	void Window::set_pixel(int const& x, int const& y, Uint32 const& color) {
		...
		rect.w = 1;
		rect.h = 1;
		SDL_FillRect(pixel, nullptr, color);
		SDL_BlitSurface(pixel, nullptr, m_surface.get_surface(), &rect);
	}
	...
	std::tuple<int, int> Window::pos() const {
		int result[2];
		SDL_GetWindowPosition(m_window.get(), result, result + 1);
		return std::tuple<int, int>(result[0], result[1]);
	}
